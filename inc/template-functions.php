 ...
 * @return array
 */
function rodolfo_rivera_body_classes( $classes ) {
	...
	if ( rodolfo_rivera_image_filters_enabled() ) {
		$classes[] = 'image-filters-enabled';
	}

	return $classes;
}
add_filter( 'body_class', 'rodolfo_rivera_body_classes' );

/**
 * Adds custom class to the array of posts classes.
 */
function rodolfo_rivera_post_classes( $classes, $class, $post_id ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'rodolfo_rivera_post_classes', 10, 3 );


/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function rodolfo_rivera_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'rodolfo_rivera_pingback_header' );

/**
 * Changes comment form default fields.
 */
function rodolfo_rivera_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'rodolfo_rivera_comment_form_defaults' );

/**
 * Filters the default archive titles.
 */
function rodolfo_rivera_get_the_archive_title() {
	if ( is_category() ) {
...
add_filter( 'get_the_archive_title', 'rodolfo_rivera_get_the_archive_title' );

/**
 * Determines if post thumbnail can be displayed.
 */
function rodolfo_rivera_can_show_post_thumbnail() {
	return apply_filters( 'rodolfo_rivera_can_show_post_thumbnail', ! post_password_required() && ! is_attachment() && has_post_thumbnail() );
}

/**
 * Returns true if image filters are enabled on the theme options.
 */
function rodolfo_rivera_image_filters_enabled() {
 ...
 */
function rodolfo_rivera_post_thumbnail_sizes_attr( $attr ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'rodolfo_rivera_post_thumbnail_sizes_attr', 10, 1 );

/**
 * Returns the size for avatars used in the theme.
 */
function rodolfo_rivera_get_avatar_size() {
	return 60;
}
 ...
 */
function rodolfo_rivera_is_comment_by_post_author( $comment = null ) {
 ...
 */
function rodolfo_rivera_get_discussion_data() {
 ...
 */
function rodolfo_rivera_add_ellipses_to_nav( $nav_menu, $args ) {
	...
	if ( 'menu-1' === $args->theme_location ) :
		...
		$nav_menu .= rodolfo_rivera_get_icon_svg( 'arrow_drop_down_ellipsis' );
		$nav_menu .= '</button>';
		...
		$nav_menu .= rodolfo_rivera_get_icon_svg( 'chevron_left' );
...
add_filter( 'wp_nav_menu', 'rodolfo_rivera_add_ellipses_to_nav', 10, 2 );
 ...
 */
function rodolfo_rivera_nav_menu_link_attributes( $atts, $item, $args, $depth ) {
...
add_filter( 'nav_menu_link_attributes', 'rodolfo_rivera_nav_menu_link_attributes', 10, 4 );
 ...
 * Add a dropdown icon to top-level menu items
 */
function rodolfo_rivera_add_dropdown_icons( $output, $item, $depth, $args ) {
	...
	if ( in_array( 'mobile-parent-nav-menu-item', $item->classes, true ) && isset( $item->original_id ) ) {
		...
		$link = sprintf(
			...
			rodolfo_rivera_get_icon_svg( 'chevron_left', 24 )
		);
		...
		$icon = rodolfo_rivera_get_icon_svg( 'keyboard_arrow_down', 24 );
...
add_filter( 'walker_nav_menu_start_el', 'rodolfo_rivera_add_dropdown_icons', 10, 4 );
 ...
 */
function rodolfo_rivera_add_mobile_parent_nav_menu_items( $sorted_menu_items, $args ) {
	static $pseudo_id = 0;
	if ( ! isset( $args->theme_location ) || 'menu-1' !== $args->theme_location ) {
...
add_filter( 'wp_nav_menu_objects', 'rodolfo_rivera_add_mobile_parent_nav_menu_items', 10, 2 );

/**
 * Convert HSL to HEX colors
 */
function rodolfo_rivera_hsl_hex( $h, $s, $l, $to_hex = true ) {
